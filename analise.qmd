...
Os participantes ($n = 61$) foram divididos em dois grupos:

- O grupo `aula` ($n_\text{aula} = 25$) recebeu uma aula expositiva.
- O grupo `jogo` ($n_\text{jogo} = 36$) participou de uma sessão do jogo.
...
dot_bars %>% 
  ...
  geom_point(
    aes(y = grupo, x = média, color = grupo),
    size = 3
  ) +
  scale_x_continuous(
    limits = c(40, 80),
